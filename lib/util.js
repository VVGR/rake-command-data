...
var rapServerUrl = 'http://192.168.1.202/jedi_rap/?project_id=';

module.exports = _;

_.updateData = function( rapId ){
    var url = rapServerUrl + rapId;
...
_.processProjectData = function( projectData ){
    ...
    // 原本的 conf 内容
    var conf_file = _.resolveFilePath( 'server.base.conf' );

    fs.readFile( conf_file, 'utf-8', function( err, data ){
        if ( err ) {
            console.log( err );
        } else {
            confData = data + endOfLine + confData;
...
_.collectServerConf = function( actionData ){
    ...
    function makeRegRule( url ){
        ...
        var reg = url.replace(/\/|\.|\-/g, function( chr ){
            return matchMap[ chr ];
        });
        return '^'+ reg +'$'
    }
...
_.generateMockDataFiles = function( actionData ){
    var tpl = actionData.tpl;
    var mockDataFileName = tpl.replace('.tpl', '.json');
    var data = JSON.stringify( actionData.mockData );
    fs.writeFile( path.join( fis.project.getProjectPath(), 'test', mockDataFileName ), data, function( err ){
        if ( err ) {
            throw err;
        }
        console.log( mockDataFileName + ' created');
    });
};
